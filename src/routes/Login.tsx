...
export function Login() {
  ...
  const handleAuthnLogin = () => {
    ...
    if (usernameEmail.current)
      formData.append("email", usernameEmail.current.value)
    if (password.current) formData.append("password", password.current.value)
    console.log("password: ", password.current)
    console.log("usernameEmail: ", usernameEmail.current)
    ...
    fetch(`http://localhost:3000/webauthn/beginlogin`, options)
      ...
      .then((credentialRequestOptions) => {
        userUid = credentialRequestOptions.userUid
      ...
      .then((assert) => {
        ...
        console.log("userUid: ", userUid)
        fetch(`http://localhost:3000/webauthn/finishlogin`, {
          method: "POST",
          mode: "cors",
          body: JSON.stringify({
            userUid,
